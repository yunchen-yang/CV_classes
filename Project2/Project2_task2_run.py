...
path=sys.path[0]+'\\'    
img1=cv2.imread(path+"tsucuba_left.png")
img2=cv2.imread(path+"tsucuba_right.png")
...
stereo=cv2.StereoBM_create(64,27)
disparity=stereo.compute(img1_g,img2_g)

cv2.imwrite(path+"task2_sift1.jpg",img1_k)
cv2.imwrite(path+"task2_sift2.jpg",img2_k)
cv2.imwrite(path+"task2_matches_knn.jpg",img_match_knn)

cv2.imwrite(path+"Matching_homo_rand.jpg",img_homo_rand)
cv2.imwrite(path+"task2_epi_right.jpg",img1_ep)
cv2.imwrite(path+"task2_epi_left.jpg",img2_ep)

plt.imsave(path+'task2_disparity.jpg',disparity,cmap='gray')
