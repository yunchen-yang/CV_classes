...
def hough_draw_round(img,votemap,R1,R2,T,t_a,t_b,t_r):
    ...
    for dn in range(0,len(selected)):               
        img_draw=cv2.circle(img_draw,(pl_coor[1][dn],pl_coor[0][dn]),pl_coor[2][dn],(0,0,255),1)
    return img_draw
path=sys.path[0]+'\\' 
...
#l=len(np.where(map_edge==255)[0])
cv2.imshow('map_edge',map_edge)
votemap_edge=round_hough(map_edge,22,25)


votemap=votemap_edge
T=50
R1=22
R2=25
t_a=2
...
for dn in range(0,len(selected)):               
    img_draw=cv2.circle(img_draw,(pl_coor[1][dn],pl_coor[0][dn]),pl_coor[2][dn],(0,0,255),1)
#img_d=hough_draw_round(img_color,votemap_edge,22,25,50,2,2,2)
