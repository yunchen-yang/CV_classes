...
def round_shape(dim):
    ...
    for x in range(0,dim):
        for y in range(0,dim):
            if np.ceil(pow(((x-(dim-1)/2)*(x-(dim-1)/2)+(y-(dim-1)/2)*(y-(dim-1)/2)),0.5))<radius:
                mat[x,y]=255
...
#img_ero=erosion(img_no,knl_cross,ori)
#img_dila=dilation(img_no,knl_cross,ori)
...
cv2.imshow('res_noise1',res_noise1)
cv2.imshow('res_noise2',res_noise2)
cv2.imshow('res_bound1',res_bound1)
cv2.imshow('res_bound2',res_bound2)