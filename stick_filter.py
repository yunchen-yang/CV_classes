...
img_grey=sampling(img_grey_o,4)
Gau_f=gaus_kernel(sqrt(2),7)
img=image_filter(img_grey,Gau_f)
##
#clustering of image
img_clust=kmeans_img(img,5)
#define kernel matrix for as image filter
sobelv=array([[1,0,-1],[2,0,-2],[1,0,-1]])
sobelh=transpose(sobelv)
##
#operate filters on images
edge_vertical=image_filter(img,sobelv)
edge_vm=edge_vertical.astype(int)
edge_horizontal=image_filter(img,sobelh)
edge_hm=edge_horizontal.astype(int)
##
#calculate the gradient on each pixel
length=img.shape[0]
width=img.shape[1]
edge=zeros((img.shape))
for x in range(0,length):
    for y in range(0,width):
        edge[x,y]=sqrt(edge_vm[x,y]*edge_vm[x,y]+edge_hm[x,y]*edge_hm[x,y])
